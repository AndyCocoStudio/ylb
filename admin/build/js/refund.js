(function () {
    ...
    var m = {
        ...
        filterList: function () {
            $.when($.ajax({
                url: $.apiUrl + "/order/refund?k=0&m=" + refund.filter.m + "&et=" + refund.filter.et + "&st=" + refund.filter.st + "&en=" + refund.filter.en + "&cp=" + refund.cp + "&sz=" + refund.sz,
                type: "GET"
            })).done(function (d) {
                $.ylbAjaxHandler(d, function () {
                    refund.list = d.data;
        ...
        buildVue: function () {
            refund = new Vue({
                ...
                methods: {
                    ...
                    agree: function (id) {
                        var c = confirm("确认同意该笔退款？");
                        if (c) {
                            $.ajax({
                                url: $.apiUrl + "/order/refund/agree",
                                type: "POST",
